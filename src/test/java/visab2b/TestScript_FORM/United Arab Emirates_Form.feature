...
* def validations = validation_Messages.Validations
  ...
  Scenario: Positive flow of the transaction
    ...
    * reqadd.params.Payload.CreditorAgent.Bic = "CBAUAEAAXXX"
    * reqadd.params.Payload.InstructedAmount.Currency = "AED"
    * reqadd.params.Payload.CreditorAccount.IBan = "AE070331234567890123456"
...
* def sleep = function(millis){ return java.lang.Thread.sleep(millis) }
  ...
  Scenario: Payment purpose code empty
    * def user = testData.Visa_Mk
    Given url QaUrl + 'api'
    * def reqadd = read('classpath:visab2b/Payload/form_Review_transaction.json')
    * reqadd.params.Payload.CreditorAgent.Bic = "CBAUAEAAXXX"
