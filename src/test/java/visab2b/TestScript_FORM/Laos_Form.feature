...
* def validations = validation_Messages.Validations
  ...
  Scenario: Positive flow of the transaction
    ...
    * reqadd.params.Payload.CreditorAgent.Bic = "LPDRLALAXXX"
    * reqadd.params.Payload.InstructedAmount.Currency = "LAK"
    ...
    * def value = signsreq(approve,user)
    ...
    When method POST
    Then status 200
    * jutil.SetData("GetAuditsLaosformpostive",referencenumber)
...
* def sleep = function(millis){ return java.lang.Thread.sleep(millis) }
  ...
  Scenario: Positive flow with other currency
    ...
    * def value = signsreq(approve,user)
    ...
    When method POST
    Then status 200
    * jutil.SetData("GetAuditsLaosformothercurrency",referencenumber)
    * jutil.SetData("Duplicaterefnumber",referencenumber)
    * print jutil.SetData('Duplicaterefnumber',referencenumber)
