...
* def validations = validation_Messages.Validations
  ...
  Scenario: Positive flow of the transaction
    ...
    * reqadd.params.Payload.CreditorAgent.Bic = "ISBKTRISXXX"
    * reqadd.params.Payload.InstructedAmount.Currency = "TRY"
    * reqadd.params.Payload.CreditorAccount.Iban = "TR330006100519786457841326"
    ...
    * def value = signsreq(approve,user)
    ...
    When method POST
    Then status 200
    * jutil.SetData("GetAuditsTurkeyformpostive",referencenumber)
...
* def sleep = function(millis){ return java.lang.Thread.sleep(millis) }
  ...
  Scenario: Positive flow with USD currency
    ...
    * def value = signsreq(approve,user)
    ...
    When method POST
    Then status 200
    * jutil.SetData("GetAuditsTurkeyformothercurrency",referencenumber)
    * jutil.SetData("Duplicaterefnumber",referencenumber)
    * print jutil.SetData('Duplicaterefnumber',referencenumber)
