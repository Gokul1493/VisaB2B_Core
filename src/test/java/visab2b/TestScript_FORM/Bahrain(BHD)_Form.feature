...
* def sleep = function(millis){ return java.lang.Thread.sleep(millis) }
  ...
  Scenario: Using 2 digit of payment purpose code
    * def user = testData.Visa_Mk
    Given url QaUrl + 'api'
    * def reqadd = read('classpath:visab2b/Payload/form_Review_transaction.json')
    * reqadd.params.Payload.InstructedAmount.Currency = "USD"
    * reqadd.params.Payload.CreditorAccount.Iban = "BH67BMAG00001299123456"
    * reqadd.params.Payload.CreditorAgent.Bic = "NBOBBHBMXXX"
    * reqadd.params.Payload.PaymentPurposeCode = "CB"
    * reqadd.params.Api.Credential = testData.Visa_Mk.Credential
    * reqadd.params.Api.deviceId = testData.Visa_Mk.keyId
    * def value = signsreq(reqadd,user)
    And request value
    When method POST
    Then status 200
    * match response.error.message contains validations.LessmorePOP_BahrainAny

  Scenario: Using 4 digit of payment purpose code
    * def user = testData.Visa_Mk
    Given url QaUrl + 'api'
    * def reqadd = read('classpath:visab2b/Payload/form_Review_transaction.json')
    * reqadd.params.Payload.InstructedAmount.Currency = "USD"
    * reqadd.params.Payload.CreditorAccount.Iban = "BH67BMAG00001299123456"
    * reqadd.params.Payload.CreditorAgent.Bic = "NBOBBHBMXXX"
    * reqadd.params.Payload.PaymentPurposeCode = "CBRT"
    * reqadd.params.Api.Credential = testData.Visa_Mk.Credential
    * reqadd.params.Api.deviceId = testData.Visa_Mk.keyId
    * def value = signsreq(reqadd,user)
    And request value
