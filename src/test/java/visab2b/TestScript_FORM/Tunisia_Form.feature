...
* def validations = validation_Messages.Validations
  ...
  Scenario: Positive flow of the transaction
    ...
    * reqadd.params.Payload.CreditorAgent.Bic = "ATBKTNTTXXX"
    * reqadd.params.Payload.InstructedAmount.Currency = "TND"
    * reqadd.params.Payload.CreditorAccount.Iban = "TN5910006035183598478831"
    ...
    * def value = signsreq(approve,user)
    ...
    When method POST
    Then status 200
    * jutil.SetData("GetAuditsTunisiaformpostive",referencenumber)
...
* def sleep = function(millis){ return java.lang.Thread.sleep(millis) }
  ...
  Scenario: Positive flow with usd currerncy
    * def user = testData.Visa_Mk
    Given url QaUrl + 'api'
    * def reqadd = read('classpath:visab2b/Payload/form_Review_transaction.json')
    * reqadd.params.Payload.CreditorAgent.Bic = "ATBKTNTTXXX"
    * reqadd.params.Payload.InstructedAmount.Currency = "USD"
    * reqadd.params.Payload.CreditorAccount.AccountNumber = "232243546575638"
    * reqadd.params.Api.Credential = testData.Visa_Mk.Credential
    * reqadd.params.Api.deviceId = testData.Visa_Mk.keyId
    * def value = signsreq(reqadd,user)
    And request value
    ...
    * def value = signsreq(approve,user)
    ...
    When method POST
    Then status 200
    * jutil.SetData("GetAuditsTunisiaformothercurrency",referencenumber)
    * jutil.SetData("Duplicaterefnumber",referencenumber)
    * print jutil.SetData('Duplicaterefnumber',referencenumber)
