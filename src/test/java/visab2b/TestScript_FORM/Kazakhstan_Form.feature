Feature: swift_file_transfer_Form_belarus
...
* def validations = validation_Messages.Validations
  ...
  Scenario: Positive flow of the transaction
    ...
    * reqadd.params.Payload.CreditorAccount.AccountNumber = "213487452154"
    * reqadd.params.Payload.CreditorAgent.Bic = "HLALKZKZXXX"
    ...
    * jutil.SetData("GetAuditPositive_Kazakhstan",referencenumber)
...
* def sleep = function(millis){ return java.lang.Thread.sleep(millis) }
  ...
  ## Maker user intiate the transaction
  Scenario: Transaction initiated for Kazakhstan country with different currency
    * def user = testData.Visa_Mk
    Given url QaUrl + 'api'
    * def reqadd = read('classpath:visab2b/Payload/form_Review_transaction.json')
    ...
    * jutil.SetData("Duplicaterefnumber",referencenumber)
    * print jutil.SetData('Duplicaterefnumber',referencenumber)
    * jutil.SetData("GetAuditCrossBorder_Kazakhstan",referencenumber)
