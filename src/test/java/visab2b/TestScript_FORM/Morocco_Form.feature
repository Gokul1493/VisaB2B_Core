...
* reqadd.params.Payload.CreditorAgent.Bic = "CDMAMAMCXXX"
* reqadd.params.Payload.InstructedAmount.Currency = "MAD"
* reqadd.params.Payload.CreditorAccount.AccountNumber = "689054364376585782323222"
 ...
 * print referencenumber
 ...
 When method POST
 Then status 200
   * jutil.SetData("GetAuditsMoroccoformpostive",referencenumber)
...
* match response.error.message contains validations.EmptyIban_AccNumber	
	...
	Scenario: 23 digit of account number
 * def user = testData.Visa_Mk
 Given url QaUrl + 'api'
 * def reqadd = read('classpath:visab2b/Payload/form_Review_transaction.json')
* reqadd.params.Payload.CreditorAgent.Bic = "CDMAMAMCXXX"
* reqadd.params.Payload.InstructedAmount.Currency = "MAD"
* reqadd.params.Payload.CreditorAccount.AccountNumber = "68905436437658578232322"
 * reqadd.params.Api.Credential = testData.Visa_Mk.Credential
 * reqadd.params.Api.deviceId = testData.Visa_Mk.keyId
 * def value = signsreq(reqadd,user)
 And request value
 When method POST
 Then status 200
* match response.error contains {message:"Creditor AccountNumber must be 24-numeric.",data:null}	


	Scenario: 25 digit of account number
 * def user = testData.Visa_Mk
 Given url QaUrl + 'api'
 * def reqadd = read('classpath:visab2b/Payload/form_Review_transaction.json')
* reqadd.params.Payload.CreditorAgent.Bic = "CDMAMAMCXXX"
* reqadd.params.Payload.InstructedAmount.Currency = "MAD"
* reqadd.params.Payload.CreditorAccount.AccountNumber = "6890543643765857823232225"
 * reqadd.params.Api.Credential = testData.Visa_Mk.Credential
 * reqadd.params.Api.deviceId = testData.Visa_Mk.keyId
 * def value = signsreq(reqadd,user)
 And request value
 ...
 * print referencenumber
 ...
 When method POST
 Then status 200
   * jutil.SetData("GetAuditsMoroccoformothercurrency",referencenumber)
 * jutil.SetData("Duplicaterefnumber",referencenumber)
	* print jutil.SetData('Duplicaterefnumber',referencenumber)
