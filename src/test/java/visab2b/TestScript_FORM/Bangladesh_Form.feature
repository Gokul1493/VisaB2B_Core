Feature: swift_file_transfer_Form_albania
  ...
  Scenario: Positive flow of the transaction
    ...
    * reqadd.params.Payload.InstructedAmount.Currency = "BDT"
    * reqadd.params.Payload.CreditorAccount.AccountNumber = "68539007547034666"
    * reqadd.params.Payload.CreditorAgent.BranchId = "Branch 123"
    * reqadd.params.Payload.CreditorAgent.Bic = "PUBABDDH216"
    ...
    * def value = signsreq(approve,user)
    ...
    When method POST
    Then status 200
    * jutil.SetData("GetAuditsBangladeshformpostive",referencenumber)
  ...
  Scenario: Positive flow with other currenecy
    * def user = testData.Visa_Mk
    Given url QaUrl + 'api'
    * def reqadd = read('classpath:visab2b/Payload/form_Review_transaction.json')
    ...
    * def value = signsreq(approve,user)
    ...
    When method POST
    Then status 200
    * jutil.SetData("GetAuditsBangladeshFormothercurrency",referencenumber)
    * jutil.SetData("Duplicaterefnumber",referencenumber)
    * print jutil.SetData('Duplicaterefnumber',referencenumber)
