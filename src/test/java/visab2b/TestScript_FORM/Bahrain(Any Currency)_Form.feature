...
* def validations = validation_Messages.Validations
  ...
  Scenario: Positive flow of the transaction
    ...
    * reqadd.params.Payload.CreditorAccount.Iban = "BH67BMAG00001299123456"
    * reqadd.params.Payload.CreditorAgent.Bic = "NBOBBHBMXXX"
    * reqadd.params.Payload.PaymentPurposeCode = "CBP"
    ...
    * def value = signsreq(approve,user)
    ...
    When method POST
    Then status 200
    * jutil.SetData("GetAuditPositive_Bahrain(Any Currency)",referencenumber)
...
* def sleep = function(millis){ return java.lang.Thread.sleep(millis) }
  ...
  Scenario: PaymentPurposeCode as empty
    * def user = testData.Visa_Mk
    Given url QaUrl + 'api'
    * def reqadd = read('classpath:visab2b/Payload/form_Review_transaction.json')
    * reqadd.params.Payload.InstructedAmount.Currency = "USD"
  ...
  Scenario: Using Invalid Currency
    * def user = testData.Visa_Mk
    Given url QaUrl + 'api'
    * def reqadd = read('classpath:visab2b/Payload/form_Review_transaction.json')
    * reqadd.params.Payload.CreditorAgent.Bic = "ABOCVNVXXXX"
    * reqadd.params.Payload.InstructedAmount.Currency = "ABC"
    * reqadd.params.Payload.CreditorAccount.AccountNumber = "232243546575638476867869"
    * reqadd.params.Payload.CreditorAgent.BranchName = "Vietcombank"
  ...
  Scenario: Positive flow using different currency
    * def user = testData.Visa_Mk
    Given url QaUrl + 'api'
    * def reqadd = read('classpath:visab2b/Payload/form_Review_transaction.json')
    ...
    * def value = signsreq(approve,user)
    ...
    When method POST
    Then status 200
    * jutil.SetData("GetAuditCrossBorder_Bahrain(Any Currency)",referencenumber)
    * jutil.SetData("Duplicaterefnumber",referencenumber)
    * print jutil.SetData('Duplicaterefnumber',referencenumber)
