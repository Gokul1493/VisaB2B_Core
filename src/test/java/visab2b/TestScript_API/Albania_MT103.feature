Feature: swift_file_transfer_MT103
...
Scenario: Transaction with end to end identification already exists
  ...
  * def value = signsreq(reqadd,user)
	...
	When method POST
	Then status 200
	* match response.error contains {message:"Transaction with end to end identification already exists",data:null}
	
Scenario: Transaction with end to end as Empty
	* def content = read('classpath:visab2b/MT103_files/ALBANIA.txt')
	* def finalMt103 = content.replaceAll("20:AUTOMT103SFTPALBENIA", "20:"  )
  * print finalMt103
  * def user = testData.Visa_Mk
  Given url QaUrl + 'api'
  * def reqadd = read('classpath:visab2b/Payload/MT103_Review_transaction.json')
  ...
  * def value = signsreq(reqadd,user)
	...
	When method POST
	Then status 200
	* match response.error contains {message:"Parameter 'EndToEndIdentification' cannot be null.",data:null}
	
Scenario: Creditor Bic as Empty
	 * def content = read('classpath:visab2b/MT103_files/ALBANIA.txt')
	 * def finalMt103 = content.replaceAll("70:/POP/Payment as agreed", "70:"  )
   * print finalMt103
   * def user = testData.Visa_Mk
   Given url QaUrl + 'api'
   * def reqadd = read('classpath:visab2b/Payload/MT103_Review_transaction.json')
   ...
   * def value = signsreq(reqadd,user)
	 ...
	 When method POST
	 Then status 200
	 * match response.error contains {message:"Parameter 'PaymentPurpose' cannot be null.",data:null}
...
Scenario: DebtorAccountNumber as Empty
   ...
   * def value = signsreq(reqadd,user)
	 ...
	 When method POST
	 Then status 200
	 * match response.error contains {message:"Parameter 'DebtorAccountNumber' cannot be null.",data:null}
...
Scenario: DebtorName as Empty
  ...
  * def value = signsreq(reqadd,user)
	...
	When method POST
	Then status 200
	* match response.error contains {message:"Parameter 'DebtorName' cannot be null.",data:null}
	
Scenario: Creditor Bic as Empty
	* def content = read('classpath:visab2b/MT103_files/ALBANIA.txt')
	* def finalMt103 = content.replaceAll(":57A:PUPPALTRXXX", ":57A:")
  * print finalMt103
  * def user = testData.Visa_Mk	
  Given url QaUrl + 'api'
  * def reqadd = read('classpath:visab2b/Payload/MT103_Review_transaction.json')
  ...
  * def value = signsreq(reqadd,user)
	...
	When method POST
	Then status 200
	* match response.error contains {message:"Invalid BIC",data:null}
	
Scenario: Creditor Bic as Invaild
	* def content = read('classpath:visab2b/MT103_files/ALBANIA.txt')
	* def finalMt103 = content.replaceAll(":57A:PUPPALTRXXX", ":57A:ABCD")
  * print finalMt103
  * def user = testData.Visa_Mk	
  Given url QaUrl + 'api'
  * def reqadd = read('classpath:visab2b/Payload/MT103_Review_transaction.json')
  ...
  * def value = signsreq(reqadd,user)
	...
	When method POST
	Then status 200
	* match response.error contains {message:"Creditor BIC invalid.",data:null}
	

Scenario: CreditorIBAN/AccountNumber as Empty
	* def content = read('classpath:visab2b/MT103_files/ALBANIA.txt')
	* def finalMt103 = content.replaceAll("59:/AL47212110090000000235698741", "59:"  )
  * print finalMt103
  * def user = testData.Visa_Mk
  Given url QaUrl + 'api'
  * def reqadd = read('classpath:visab2b/Payload/MT103_Review_transaction.json')
  ...
  * def value = signsreq(reqadd,user)
	...
	When method POST
	Then status 200
	* match response.error contains {message:"CreditorAccountNumber/CreditorIBAN is cannot be empty",data:null}
...
Scenario: Transaction initated for albania country with different currency  (currency:- USD)
  ...
  * print referencenumber
  * def finalMt103 = content.replaceAll("20:AUTOMT103SFTPALBENIA", "20:" + referencenumber ).replaceAll(":32A:200408ALL138.28", ":32A:200408USD138.28").replaceAll("70:/POP/Payment as agreed", "70:"  )
  * print finalMt103
  * def user = testData.Visa_Mk
  Given url QaUrl + 'api'
  * def reqadd = read('classpath:visab2b/Payload/MT103_Review_transaction.json')
