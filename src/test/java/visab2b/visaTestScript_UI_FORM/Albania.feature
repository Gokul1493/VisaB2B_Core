Feature: Test the Visab2b transactions For Albania Country
  ...
  Background: 
    ...
    * def req = read('classpath:Testdata/credentials.json')
    * def Random = Java.type('java.util.Random')
    * def random = new Random()
    * def randomNumber = String((new Random()).nextInt(100000) + 1)
    * print 'Random Number:', randomNumber
    * def randomNumberFromAlbania = Driver.GetData("ReferenceNo")
    * print randomNumberFromAlbania
  ...
  Scenario: Login into Visab2b maker
    ...
    * waitFor('input[id=j_username]').input(req.UserName)
    ...
    * waitFor('{span}Beneficiary Name').input("Albani")
    * delay(2000)
    * waitFor('{span}Beneficiary Bic').click()
    * waitFor('{span}Beneficiary Bic').input("ASDFAL50")
    * waitFor('{span}Beneficiary Iban').click()
    * waitFor('{span}Beneficiary Iban').input("AL5202366")
    * waitFor('{span}Beneficiary Address').click()
    * waitFor('{span}Beneficiary Address').input("Albania street")
    * delay(2000)
    #Transaction details
  ...
  Scenario: validate response
    ...
    * waitFor('{}EndToEnd ID').input(randomNumberFromAlbania)
    * delay(10000)
