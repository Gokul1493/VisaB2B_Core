Feature: Test the Visab2b transactions For Albania Country
  ...
  Background: 
    * def Url = "https://qavisab2b.netxd.com/visab2b/#/login"
    ...
    * def randomNumberForAlgeria = Driver.GetData("ReferenceNo")
    * print randomNumberForAlgeria

  @ignore
  Scenario: 
    Given url Url
    * req.params.Api.Credential = testData.Visa.credentials
    * req.params.Payload.user.emailId
    * req.params.Payload.user.password
    And request req
    When method POST
    Then status 200
  ...
  Scenario: Login into Visab2b maker
    ...
    * print loginObect.login.userNamefield
    ...
    * waitFor('{span}MRF SPORTS').click()
    * delay(2000)
    #Beneficiary details
    #* waitFor('//input[@id='mat-input-0']').input("NICK")
    * waitFor('{span}Beneficiary Name').click()
    * waitFor('{span}Beneficiary Name').input("ALGAN")
    * delay(2000)
    * waitFor('{span}Beneficiary BIC').click()
    * waitFor('{span}Beneficiary BIC').input("ASDFDZ50")
    * waitFor('{span}Beneficiary Contact Phone Number').click()
    * waitFor('{span}Beneficiary Contact Phone Number').input("+1-33366")
    * waitFor('{span}Beneficiary Account Number').click()
    * waitFor('{span}Beneficiary Account Number').input("21314548796422154878")
    * waitFor('{span}Beneficiary ContactName').click()
    * waitFor('{span}Beneficiary ContactName').input("DZNAME")
    * waitFor('{span}Beneficiary Address').click()
    * waitFor('{span}Beneficiary Address').input("BRITIAN street")
    * delay(2000)
    #Transaction details
    ...
    * waitFor('{span}Purpose').input("payment")
  ...
  Scenario: validate response
    ...
    * waitFor('{}EndToEnd ID').input(randomNumberForAlgeria)
    * delay(10000)
