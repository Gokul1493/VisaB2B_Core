...
* print Accno
  ...
  * approvetrasnaction.params.Api.deviceId = testData.Visa_CK.keyId
	...
	* match response.result.message == validations.successMessage
	...
	## Maker user intiate the transaction
 Scenario: Positive flow of the transaction With Clearing Member ID only
  * def content = read('classpath:visab2b/MT103_files/CANADACAD.txt')
  * def referencenumber = "AUTOMT103SFTPCANADACAD" + Accno
  * print referencenumber
  ...
  * approvetrasnaction.params.Api.deviceId = testData.Visa_CK.keyId
	...
	* match response.result.message == validations.successMessage
	...
	Scenario: Transaction initated for CANADACAD country with different currency(without BIC ))  (currency:- ALL)
  * def content = read('classpath:visab2b/MT103_files/CANADACAD.txt')
  * def referencenumber = "AUTOMT103SFTPCANADACAD" + Accno
  * print referencenumber
 ...
 * match response.result.message == validations.successMessage
	...
	Scenario: Transaction initated for CANADACAD country with different currency with clearing member ID  (currency:- ALl)
  * def content = read('classpath:visab2b/MT103_files/CANADACAD.txt')
  * def referencenumber = "AUTOMT103SFTPCANADACAD" + Accno
  * print referencenumber
...
Scenario: Transaction with end to end as Empty
	* def content = read('classpath:visab2b/MT103_files/CANADACAD.txt')
	* def finalMt103 = content.replaceAll("20:MT103CANADACAD20243103001", "20:"  )
  * print finalMt103
  * def user = testData.Visa_Mk
  Given url QaUrl + 'api'
  * def reqadd = read('classpath:visab2b/Payload/MT103_Review_transaction.json')
