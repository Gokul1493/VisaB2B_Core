Feature: Slovakia
  ...
  Background: 
    ...
    * beneficiaries.params.payload.beneficiary.creditorIBAN = beneTestData.Slovakia.creditorIBAN
    * beneficiaries.params.payload.beneficiary.creditorAgentBic = beneTestData.Slovakia.creditorAgentBic
    * beneficiaries.params.payload.beneficiary.creditorName = beneTestData.Slovakia.creditorName
    * beneficiaries.params.payload.beneficiary.creditorAddress = beneTestData.Slovakia.creditorAddress
    * beneficiaries.params.payload.beneficiary.instructedCurrency = beneTestData.Slovakia.instructedCurrency
  ...
  Scenario: transaction without amount
    * call read('classpath:visab2b/TestScript_corporate/baseFunctions/loginCreds.feature@loginWithMaker')
    #Get Beneficiary
    Given url 'https://connectors.cbwpayments.com/visab2b/api'
    * configure headers = {"Authorization":'#(auth)'}
    * getbeneficiary_req.params.api.deviceId = user.keyId
    ...
    * Transactions.params.payload.creditorAccount.name = beneTestData.Slovakia.creditorName
    * Transactions.params.payload.creditorAccount.creditorAgentBic = beneTestData.Slovakia.creditorAgentBic
    * Transactions.params.payload.creditorAccount.address = beneTestData.Slovakia.creditorAddress
    * Transactions.params.payload.creditorAccount.iban = beneTestData.Slovakia.creditorIBAN
    #* Transactions.params.payload.paymentPurpose= "POP"
    * def user = testData.Visa_CorporateMK
    * Transactions.params.api.deviceId = user.keyId
    * def beneficiaryid_value = response.result.beneficiaries[0].id
    ...
    * Transactions.params.payload.creditorAccount.name = beneTestData.Slovakia.creditorName
    * Transactions.params.payload.creditorAccount.iban = beneTestData.Slovakia.creditorIBAN
    * Transactions.params.payload.creditorAgent.bic = beneTestData.Slovakia.creditorAgentBic
    * def sig = signscorp(Transactions,user)
    * request Transactions
    * method POST
    * status 200
