Feature: Uruguay
  ...
  Background: 
    ...
    * beneficiaries.params.payload.beneficiary.creditorIBAN = beneTestData.Uruguay.creditorIBAN
    * beneficiaries.params.payload.beneficiary.creditorAgentBic = beneTestData.Uruguay.creditorAgentBic
    * beneficiaries.params.payload.beneficiary.creditorName = beneTestData.Uruguay.creditorName
    * beneficiaries.params.payload.beneficiary.creditorAddress = beneTestData.Uruguay.creditorAddress
    * beneficiaries.params.payload.beneficiary.creditorTaxID = beneTestData.Uruguay.creditorTaxID
    * beneficiaries.params.payload.beneficiary.instructedCurrency = beneTestData.Uruguay.instructedCurrency
    * beneficiaries.params.payload.beneficiary.creditorAgentBranchName = beneTestData.Uruguay.creditorAgentBranchName
    * beneficiaries.params.payload.beneficiary.creditorAccountNumber = beneTestData.Uruguay.creditorAccountNumber
  ...
  Scenario: positive
    ...
    * match creditorName == beneTestData.Uruguay.creditorName
    * match creditorAgentBic == beneTestData.Uruguay.creditorAgentBic
    * match creditorIBAN == beneTestData.Uruguay.creditorIBAN
    * match creditorAddress == beneTestData.Uruguay.creditorAddress
    #Select Beneficiary
    ...
    * Transactions.params.payload.paymentPurpose= "VO23456"
    ...
    * Transactions.params.payload.instructedAmount.currency = beneTestData.Uruguay.instructedCurrency
    * Transactions.params.payload.creditorAccount.TaxId = beneTestData.Uruguay.creditorTaxID
  ...
  Scenario: transaction without amount
    ...
    * Transactions.params.payload.creditorAccount.name = beneTestData.Uruguay.creditorName
    * Transactions.params.payload.creditorAccount.creditorAgentBic = beneTestData.Uruguay.creditorAgentBic
    * Transactions.params.payload.creditorAccount.address = beneTestData.Uruguay.creditorAddress
    * Transactions.params.payload.creditorAccount.iban = beneTestData.Uruguay.creditorIBAN
    * Transactions.params.payload.instructedAmount.currency = beneTestData.Uruguay.instructedCurrency
    * def user = testData.Visa_CorporateMK
    * Transactions.params.api.deviceId = user.keyId
    * def beneficiaryid_value = response.result.beneficiaries[0].id
    ...
    * Transactions.params.payload.creditorAccount.name = beneTestData.Uruguay.creditorName
    * Transactions.params.payload.creditorAccount.iban = beneTestData.Uruguay.creditorIBAN
    * Transactions.params.payload.creditorAgent.bic = beneTestData.Uruguay.creditorAgentBic
    * def sig = signscorp(Transactions,user)
    * request Transactions
    * method POST
    * status 200
  ...
  Scenario: without Account Number
    ...
    * def auth = "Bearer "+ bearer
    * configure headers = {"Authorization":'#(auth)'}
    * beneficiaries.params.payload.beneficiary.creditorTaxID = ""
    * def user = testData.Visa_CorporateMK
    * beneficiaries.params.api.deviceId = user.keyId
    * def sig = signscorp(beneficiaries,user)
    * request beneficiaries
    ...
    * def auth = "Bearer "+ bearer
    * configure headers = {"Authorization":'#(auth)'}
    * beneficiaries.params.payload.beneficiary.creditorTaxID = "8456127"
    * def user = testData.Visa_CorporateMK
    * beneficiaries.params.api.deviceId = user.keyId
    * def sig = signscorp(beneficiaries,user)
    * request beneficiaries
    ...
    * def auth = "Bearer "+ bearer
    * configure headers = {"Authorization":'#(auth)'}
    * beneficiaries.params.payload.beneficiary.creditorTaxID = "84561277451222"
    * def user = testData.Visa_CorporateMK
    * beneficiaries.params.api.deviceId = user.keyId
    * def sig = signscorp(beneficiaries,user)
    * request beneficiaries
