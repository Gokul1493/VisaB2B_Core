Feature: Switzerland
  ...
  Background: 
    ...
    * beneficiaries.params.payload.beneficiary.creditorIBAN = beneTestData.Switzerland.creditorIBAN
    * beneficiaries.params.payload.beneficiary.creditorAgentBic = beneTestData.Switzerland.creditorAgentBic
    * beneficiaries.params.payload.beneficiary.creditorName = beneTestData.Switzerland.creditorName
    * beneficiaries.params.payload.beneficiary.creditorAddress = beneTestData.Switzerland.creditorAddress
    * beneficiaries.params.payload.beneficiary.instructedCurrency = beneTestData.Switzerland.instructedCurrency
  ...
  Scenario: positive
    ...
    * match creditorName == beneTestData.Switzerland.creditorName
    * match creditorAgentBic == beneTestData.Switzerland.creditorAgentBic
    * match creditorIBAN == beneTestData.Switzerland.creditorIBAN
    * match creditorAddress == beneTestData.Switzerland.creditorAddress
    #Select Beneficiary
  ...
  Scenario: transaction without amount
    ...
    * Transactions.params.payload.creditorAccount.name = beneTestData.Switzerland.creditorName
    * Transactions.params.payload.creditorAccount.creditorAgentBic = beneTestData.Switzerland.creditorAgentBic
    * Transactions.params.payload.creditorAccount.address = beneTestData.Switzerland.creditorAddress
    * Transactions.params.payload.creditorAccount.iban = beneTestData.Switzerland.creditorIBAN
   * Transactions.params.payload.instructedAmount.currency = beneTestData.Switzerland.instructedCurrency
    * def user = testData.Visa_CorporateMK
    * Transactions.params.api.deviceId = user.keyId
    * def beneficiaryid_value = response.result.beneficiaries[0].id
    * Transactions.params.payload.beneficiaryId = beneficiaryid_value
    * Transactions.params.payload.creditorAccount.name = beneTestData.Switzerland.creditorName
    * Transactions.params.payload.creditorAccount.iban = beneTestData.Switzerland.creditorIBAN
    * Transactions.params.payload.creditorAgent.bic = beneTestData.Switzerland.creditorAgentBic
    * def sig = signscorp(Transactions,user)
    * request Transactions
    * method POST
    * status 200
